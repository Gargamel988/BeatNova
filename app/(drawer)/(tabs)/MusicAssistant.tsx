      ...
      >
        ...
        {isLoading && (
          <View style={{ marginBottom: hp(2) }}>
            <LoadingState message="Öneri oluşturuluyor... (30 saniyeye kadar sürebilir)" />
          </View>
        )}

        {/* Error State - Geliştirilmiş */}
        {error && (
          <View style={{ marginBottom: hp(2) }}>
            <ErrorState
              ...
              onRetry={() => {
                const query = input.trim() || "Enerjik bir playlist oluştur";
                submit({ input: query });
              }}
            />
          </View>
        ...
        {!isLoading &&
          !error &&
          (!object || (!object.songs && !object.playlistInfo)) && (
            <EmptyState
              title="Henüz Öneri Yok"
              message="Yukarıdaki hızlı erişim butonlarından birine tıklayarak veya arama kutusuna şarkı adı/türü yazarak öneriler alabilirsiniz."
              icon={Music}
            />
          )}
